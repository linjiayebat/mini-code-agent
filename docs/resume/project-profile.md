...
> 与 M5b host-pinned local stdio MCP 已发布。Shell 字符串、
> 项目可执行 Hook、OS 沙箱、remote HTTP/OAuth MCP、自动 Repair Resume、Subagent/
> Worktree 和真实凭证联调尚未实现。`v0.14.0-alpha.0` GitHub prerelease 已发布；
> Python 3.12/3.13 本地各 961 passed、10 个 Windows symlink 条件跳过、90.84%
> 分支覆盖率，Ruff/Pyright/Bandit/locked pip-audit、四组 artifact smoke、PR/main
> 跨平台 CI、tag 与 Release asset digest 验证均已通过。
>
