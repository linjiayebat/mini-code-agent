...
> Subagent 已发布；M6b host-managed Worktree implementation candidate 与单独 adoption
> approval 已完成实现，正在进行 `v0.16.0-alpha.0` 发布门禁。Shell 字符串、项目可执行
...
> 条件跳过，Python 3.13 package branch coverage 88.49%；最终 artifact、远端 CI、tag、
> prerelease 和 digest 需完成后再回填。
>
...
- 通过真实 Git/Agent/adoption 与 adversarial 测试覆盖 hostile path、ignored/untracked
  ...
  branch coverage 88.49%；最终 artifact/CI/Release 证据按发布门禁单独回填。
